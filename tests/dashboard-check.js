...
const DASHBOARD_RUNTIME_VERSION = "2.6.9";
...
const JIRA_EXPORT_FIELDS = [
    ...
    { key: "releaseDate", label: "배포일", value: (_task, page) => page?.["배포일"] ?? page?.release_date ?? page?.releaseDate ?? "" },
    { key: "actualReleaseDate", label: "Actual Release Date", value: (_task, page) => page?.actual_release_date ?? page?.actualReleaseDate ?? page?.["Actual Release Date"] ?? "" },
    { key: "estimatedQaCompletionDate", label: "Estimated QA Completion Date", value: (_task, page) => page?.estimated_qa_completion_date ?? page?.estimated_qa_completion ?? page?.estimatedQaCompletionDate ?? page?.["Estimated QA Completion Date"] ?? "" },
    { key: "targetQaCompletionDate", label: "Target QA Completion Date", value: (_task, page) => page?.target_qa_completion_date ?? page?.target_qa_completion ?? page?.targetQaCompletionDate ?? page?.["Target QA Completion Date"] ?? "" },
    ...
    { key: "category", label: "구분", value: (_task, page) => page?.category ?? "" },
    ...
    { key: "serviceNowCategory", label: "ServiceNow Category", value: (_task, page) => page?.service_now_category ?? "" },
    { key: "serviceNowCreated", label: "ServiceNow 생성일", value: (_task, page) => page?.service_now_created ?? "" },
    { key: "serviceNowUpdated", label: "ServiceNow 수정일", value: (_task, page) => page?.service_now_updated ?? "" },
    {
        key: "latestWorkLog",
        label: "최근 작업일지",
    ...
    { key: "serviceNow", label: "ServiceNow", value: (_task, page) => page?.["서비스나우"] ?? "" },
    { key: "jira", label: "Jira", value: (_task, page) => page?.jira ?? "" },
    { key: "workingNotes", label: "워킹노트", value: (_task, page) => page?.["워킹노트"] ?? "" },
    { key: "bs", label: "BS", value: (_task, page) => page?.BS ?? "" },
    { key: "fs", label: "FS", value: (_task, page) => page?.FS ?? "" },
    { key: "ds", label: "DS", value: (_task, page) => page?.DS ?? "" },
    { key: "ut", label: "UT", value: (_task, page) => page?.ut ?? "" },
    { key: "relatedDocument", label: "관련 문서", value: (_task, page) => page?.["관련 문서"] ?? "" },
    { key: "testDocument", label: "테스트 문서", value: (_task, page) => page?.["테스트 문서"] ?? "" },
    { key: "created", label: "생성일", value: (_task, page) => page?.created ?? "" },
    { key: "updated", label: "수정일", value: (_task, page) => page?.updated ?? "" }
];
...
function buildJiraExportPayload(tasks, fields) {
    ...
    return {
        ...
        html: `<table><thead><tr>${fields.map(field => `<th data-jira-field="${escapeAttribute(field.key)}">${escapeHtml(field.label)}</th>`).join("")}</tr></thead><tbody>${rows.map(row => `<tr>${row.map((value, index) => `<td data-jira-field="${escapeAttribute(fields[index].key)}">${escapeHtml(value).replace(/\r?\n/g, "<br>")}</td>`).join("")}</tr>`).join("")}</tbody></table>`
    };
}
