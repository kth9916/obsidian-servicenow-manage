...
const DEFAULT_COLUMN_WIDTHS = {
    ...
    estimatedQaCompletionDate: 160,
    targetQaCompletionDate: 160,
    actualReleaseDate: 145,
    releaseDate: 92,
    scheduledDate: 92,
    lastChecked: 92,
    latestWorkLog: 260,
...
function normalizeDateTime(value) {
    ...
    if (ddmmyyyy) {
        const timePart = ddmmyyyy[4] ? ` ${ddmmyyyy[4]}` : "";
        return `${ddmmyyyy[3]}-${ddmmyyyy[2]}-${ddmmyyyy[1]}${timePart}`;
    }

    const yyyymmdd = text.match(/^(\d{4}-\d{2}-\d{2})(?:\s+(\d{2}:\d{2})(?::\d{2})?)?/);
    if (yyyymmdd) {
        const timePart = yyyymmdd[2] ? ` ${yyyymmdd[2]}` : "";
        return `${yyyymmdd[1]}${timePart}`;
    }
...
const columns = [
    ...
    },
    {
        ...
        value: page => page.short_description || page.shortDescription || page.purpose,
        type: "text",
        filterType: "text",
        sortType: "text"
    },
    ...
    {
        key: "estimatedQaCompletionDate",
        label: "Estimated QA Completion Date",
        value: page => page.estimated_qa_completion_date || page.estimated_qa_completion || page.estimatedQaCompletionDate || page["Estimated QA Completion Date"],
        type: "dateTime",
        filterType: "date",
        sortType: "dateTime"
    },
    {
        key: "targetQaCompletionDate",
        label: "Target QA Completion Date",
        value: page => page.target_qa_completion_date || page.target_qa_completion || page.targetQaCompletionDate || page["Target QA Completion Date"],
        type: "dateTime",
        filterType: "date",
        sortType: "dateTime"
    },
    {
        ...
        value: page => page.actual_release_date || page.actualReleaseDate || page["Actual Release Date"],
        type: "dateTime",
        filterType: "date",
        sortType: "dateTime"
    },
    {
        key: "releaseDate",
        label: "배포일",
        value: page => page["배포일"] || page.release_date || page.releaseDate,
        type: "date",
        filterType: "date",
        sortType: "date"
    },
    ...
    },
    {
        ...
        label: "최근 작업일지",
        value: (_page, item) => {
            const log = item?.latestWorkLog;
            if (!log) return "";
            if (log.dateTime && log.text) return `[${log.dateTime}] ${log.text}`;
            return log.text || log.dateTime || "";
        },
        type: "latestWorkLog",
        filterType: "text",
        sortType: "text"
...
const mappedTemplateProperties = new Set([
    "id", "category", "status", "purpose", "short_description", "priority", "service_now_priority",
    "ticket_creator", "ticket_requester", "service_now_created", "assignment_group",
    "assigned_person", "service_now_category", "service_now_updated",
...
function loadSettings() {
    ...
    try {
        ...
        return {
            ...
            todoDateFrom:
                typeof saved.todoDateFrom === "string"
                    ? saved.todoDateFrom
                    : defaults.todoDateFrom,

            todoDateTo:
                typeof saved.todoDateTo === "string"
                    ? saved.todoDateTo
                    : defaults.todoDateTo,
...
async function readGeneralTodoTasks() {
    const generalFile = app.vault.getAbstractFileByPath(`${ROOT_FOLDER}/To-Do.md`)
        || app.vault.getAbstractFileByPath(`${ROOT_FOLDER}/일반 To-Do.md`);
    if (generalFile?.extension === "md") {
        try {
            const markdown = await app.vault.cachedRead(generalFile);
            return extractTodos(markdown, { file: generalFile, id: "", category: "TODO" });
        } catch (error) {
            console.error("[업무현황] 일반 To-Do 읽기 실패:", error);
        }
    }
    return [];
}

async function refreshAllTodoItems() {
    const generalTasks = await readGeneralTodoTasks();
    todoItems = [...pages.flatMap(item => item.todos || []), ...generalTasks];
}

async function touchTodoLastChecked(file) {
    if (!file) return "";
    ...
    const pageItem = typeof pages !== "undefined"
        ? pages.find(item => item.page?.file?.path === file.path)
        : null;
    if (pageItem?.page) pageItem.page["마지막확인"] = today;
    return today;
}
...
async function deleteTodo(task) {
    ...
    if (!file) throw new Error("To-Do 노트를 찾을 수 없습니다.");
    let deleted = false;
...
async function addTodo(item, content, dueDate = "", status = "pending", details = "") {
    ...
    let file = null;
    if (item?.page?.file?.path) {
        file = app.vault.getAbstractFileByPath(item.page.file.path);
    } else {
        const generalPath = `${ROOT_FOLDER}/To-Do.md`;
        file = app.vault.getAbstractFileByPath(generalPath);
        if (!file) {
            await app.vault.create(generalPath, "# 📋 일반 To-Do\n\n## ✅ To-Do\n\n");
            file = app.vault.getAbstractFileByPath(generalPath);
        }
    }
    if (!file) throw new Error("To-Do 노트를 찾을 수 없습니다.");
    ...
    await refreshAllTodoItems();
    return dateTime;
}

function todoDueInfo(task, nowValue = new Date()) {
    if (!task?.dueDate) return null;
    const hasTime = task.dueDate.includes("T");
    const now = new Date(nowValue);
    const due = new Date(hasTime ? task.dueDate : `${task.dueDate}T00:00:00`);
    if (Number.isNaN(due.getTime())) return null;
    const startOfDay = value => new Date(value.getFullYear(), value.getMonth(), value.getDate());
    const dayDifference = Math.round((startOfDay(due).getTime() - startOfDay(now).getTime()) / 86400000);
    const timeText = `${String(due.getHours()).padStart(2, "0")}:${String(due.getMinutes()).padStart(2, "0")}`;
    const dateText = `${due.getMonth() + 1}월 ${due.getDate()}일`;
    const calendarText = dayDifference === 0
        ? "오늘"
        : dayDifference === 1
            ? "내일"
            : dayDifference === -1
                ...
                : dateText;
    if (task.status === "done") {
        return { label: `✓ ${calendarText}${hasTime ? ` ${timeText}` : ""}`, tone: "done", difference: dayDifference };
    }
    if (!hasTime) {
        if (dayDifference < 0) {
    ...
    const relativeTime = minutes => {
        ...
        if (absolute < 60) return `${Math.max(1, absolute)}분`;
        if (absolute < 2880) return `${Math.ceil(absolute / 60)}시간`;
        return `${Math.max(1, Math.abs(dayDifference))}일`;
    };
    if (remainingMinutes < 0) {
        return {
            label: `⏰ ${calendarText} ${timeText} 마감 · ${relativeTime(remainingMinutes)} 지남`,
            tone: "overdue",
...
const initialGeneralTasks = await readGeneralTodoTasks();
let todoItems = [...pages.flatMap(item => item.todos || []), ...initialGeneralTasks];
...
.opus-filter-multi-select {
    position: relative;
}

.opus-filter-multi-select > summary {
    min-width: 92px;
    height: 25px;
    box-sizing: border-box;
...
.opus-filter-multi-select > summary::-webkit-details-marker {
    display: none;
}

.opus-filter-multi-select > summary::after {
    content: "⌄";
    position: absolute;
    right: 7px;
    color: var(--text-muted);
...
.opus-filter-multi-options label {
    display: flex;
    align-items: center;
    gap: 7px;
    min-height: 27px;
...
.opus-filter-multi-options label:hover {
    background: var(--background-modifier-hover);
}

.opus-filter-multi-options input {
    width: auto;
    height: auto;
    margin: 0;
}
...
.opus-todo-date-filter select,
.opus-todo-date-filter input {
    min-height: 31px;
    box-sizing: border-box;
}
...
.opus-todo-date-separator {
    color: var(--text-muted);
    font-size: 11px;
}

.opus-todo-date-clear {
    min-height: 29px;
    padding: 3px 7px;
    border: 1px solid var(--background-modifier-border);
    border-radius: 6px;
...
.opus-todo-column.is-collapsed {
    min-height: 0;
    align-self: start;
}

.opus-todo-column.is-collapsed > :not(.opus-todo-column-header) {
    display: none !important;
}
...
.opus-todo-column-toggle:hover {
    background: transparent !important;
    box-shadow: none !important;
    color: var(--interactive-accent);
}

.opus-todo-column-chevron {
    width: 12px;
    color: var(--text-muted);
    font-size: 11px;
}
...
@media (max-width: 760px) {
    ...
    .opus-todo-date-filter select,
    .opus-todo-date-filter .opus-todo-date-clear {
        grid-column: 1 / -1;
    }
...
.opus-jira-export-task input {
    flex: 0 0 auto;
}

.opus-jira-export-task-text {
    display: -webkit-box;
    min-width: 0;
    overflow: hidden;
    overflow-wrap: anywhere;
...
.opus-jira-export-task-row.expanded .opus-jira-export-task-text {
    display: block;
    overflow: visible;
    -webkit-line-clamp: unset;
}

.opus-jira-export-task-more {
    padding: 1px 4px;
    border: 0 !important;
    background: transparent !important;
    color: var(--interactive-accent);
...
.opus-jira-export-task-more:hover {
    background: transparent !important;
    box-shadow: none !important;
...
.opus-jira-export-translation label {
    align-items: center;
    display: flex;
    font-weight: 600;
    gap: 7px;
...
.opus-jira-export-translation small {
    color: var(--text-muted);
    line-height: 1.35;
    padding-left: 23px;
}

.opus-jira-export-translation.is-working small {
    color: var(--interactive-accent);
}
...
.opus-jira-export-preview [data-jira-field="no"],
.opus-jira-export-preview [data-jira-field="ticket"] {
    white-space: nowrap;
}
...
.opus-jira-export-preview [data-jira-field="description"] {
    min-width: 280px;
    max-width: 360px;
}

.opus-jira-export-preview [data-jira-field="phase"] {
    min-width: 120px;
    max-width: 180px;
}
...
function formatCell(
    ...
    switch (column.type) {
        ...
        case "latestWorkLog": {
            const log = item?.latestWorkLog;
            if (!log || (!log.text && !log.dateTime)) {
                return "";
            }
            const dateStr = log.dateTime ? `<span class="opus-latest-worklog-date">${escapeHtml(log.dateTime)}</span>` : "";
            const textStr = log.text ? `<span class="opus-latest-worklog-text">${escapeHtml(log.text)}</span>` : "";
            return `
                <div class="opus-latest-worklog-cell" title="${escapeAttribute(log.raw || log.text || '')}">
                    ${dateStr}
                    ${textStr}
                </div>
            `;
            ...
            if (
                ...
                const input = `
                    <input
                        ...
                        data-inline-date-field="${column.key === "scheduledDate" ? "작업예정일" : column.key === "releaseDate" ? "배포일" : "마지막확인"}"
                        title="날짜를 바로 수정"
                    >
                `;
...
function matchesFilter(
    ...
    const filterValues = Array.isArray(filter.value)
        ? filter.value.map(value => String(value)).filter(Boolean)
    ...
    switch (filter.operator) {
        ...
        case "equals":
            ...
            return column.filterType === "select"
                ? filterValues.includes(normalizedValue)
                : normalizedValue === filterValue;

        case "notEquals":
            return !filterValues.length || !filterValues.includes(normalizedValue);
...
function createFilterValueControl(
    ...
    if (
        ...
        details.className = "opus-filter-multi-select";
        const summary = document.createElement("summary");
        const list = document.createElement("div");
        list.className = "opus-filter-multi-options";
        ...
        updateSummary();
        for (const value of getUniqueValues(column)) {
            const label = document.createElement("label");
            const checkbox = document.createElement("input");
            checkbox.type = "checkbox";
            checkbox.checked = selected.has(String(value));
            checkbox.addEventListener("change", () => {
                checkbox.checked ? selected.add(String(value)) : selected.delete(String(value));
...
function openTodoCreateModal(preselectedItem = null, preselectedStatus = "pending") {
    ...
    ticketLabel.innerHTML = `티켓 <b class="opus-todo-form-required">선택</b>`;
    const ticketSelect = document.createElement("select");
    const emptyOption = document.createElement("option");
    emptyOption.value = "";
    emptyOption.textContent = "티켓 선택 안 함 (일반 To-Do)";
    ...
    saveButton.addEventListener("click", async () => {
        const selectedItem = pages.find(item => item.page.file.path === ticketSelect.value) || null;
...
function openTodoDetailModal(task, afterSaved = null) {
    ...
    const title = document.createElement("h4");
    title.className = "opus-note-title";
    title.textContent = task.ticketId ? `☑ ${task.ticketId} To-Do` : "☑ To-Do";
    const closeButton = document.createElement("button");
    closeButton.className = "opus-note-close";
    closeButton.textContent = "×";
    header.append(title, closeButton);
    ...
    const render = () => {
        ...
        saveButton.addEventListener("click", async () => {
            ...
            try {
                ...
                new Notice(`${task.ticketId || "To-Do"}를 수정했습니다.`);
            } catch (error) {
        ...
        deleteButton.addEventListener("click", async () => {
            if (!window.confirm(`${task.ticketId ? `${task.ticketId}의 ` : ""}이 To-Do를 삭제하시겠습니까?\n\n${task.text || ""}`)) return;
            deleteButton.disabled = true;
            try {
                await deleteTodo(task);
                if (typeof afterSaved === "function") await afterSaved({ ...task, deleted: true });
                new Notice(`${task.ticketId || "To-Do"}를 삭제했습니다.`);
                close();
                renderTodoBoard();
            } catch (error) {
...
function openWorkLogModal(item) {
    ...
    const addInput = typeof sharedPlugin?.createRichMarkdownInput === "function"
        ? sharedPlugin.createRichMarkdownInput(
            addArea,
            "",
            item.page.file.path,
            "새 작업 내용을 입력하세요."
    ...
    addInput.classList.add("opus-add-log-input");

    if (addInput instanceof HTMLTextAreaElement) {
        addInput.placeholder = "새 작업 내용을 입력하세요.";
    }
...
async function changeTodoStatus(task, nextStatus, sourceElement = null) {
    ...
    try {
        ...
        new Notice(`${task.ticketId || "To-Do"} 할 일을 '${label}' 상태로 변경했습니다.`);
    } catch (error) {
...
function createTodoCard(task, showTicket) {
    ...
    if (task.details) {
        ...
        const sharedPlugin = app.plugins.getPlugin("servicenow-manage");
        if (typeof sharedPlugin?.renderMarkdownInto === "function") {
            void sharedPlugin.renderMarkdownInto(detail, task.details, task.filePath || "");
        } else {
...
const JIRA_EXPORT_FIELDS = [
    ...
    { key: "phase", label: "Phase", jiraLabel: "Phase", default: true, value: (_task, page) => page?.status ?? "" },
    ...
    { key: "lastChecked", label: "Last Checked (마지막확인)", jiraLabel: "Last Checked", value: (_task, page) => page?.["마지막확인"] ?? "" },
    { key: "priority", label: "Priority (업무 Priority)", jiraLabel: "Priority", value: (_task, page) => page?.priority ?? "" },
    { key: "serviceNowPriority", label: "ServiceNow Priority", jiraLabel: "ServiceNow Priority", value: (_task, page) => page?.service_now_priority ?? "" },
    { key: "purpose", label: "Purpose", jiraLabel: "Purpose", value: (_task, page) => page?.purpose ?? "" },
    { key: "category", label: "Category (구분)", jiraLabel: "Category", value: (_task, page) => page?.category ?? "" },
    { key: "assignmentGroup", label: "Assignment Group", jiraLabel: "Assignment Group", value: (_task, page) => page?.assignment_group ?? "" },
    { key: "assignedPerson", label: "Assigned Person", jiraLabel: "Assigned Person", value: (_task, page) => page?.assigned_person ?? "" },
    { key: "ticketCreator", label: "Ticket Creator", jiraLabel: "Ticket Creator", value: (_task, page) => page?.ticket_creator ?? "" },
    { key: "ticketRequester", label: "Ticket Requester", jiraLabel: "Ticket Requester", value: (_task, page) => page?.ticket_requester ?? "" },
    { key: "serviceNowCategory", label: "ServiceNow Category", jiraLabel: "ServiceNow Category", value: (_task, page) => page?.service_now_category ?? "" },
    { key: "serviceNowCreated", label: "ServiceNow Created (생성일)", jiraLabel: "ServiceNow Created", value: (_task, page) => page?.service_now_created ?? "" },
    { key: "serviceNowUpdated", label: "ServiceNow Updated (수정일)", jiraLabel: "ServiceNow Updated", value: (_task, page) => page?.service_now_updated ?? "" },
    {
        key: "latestWorkLog",
        label: "Recent Work Note (최근 작업일지)",
        jiraLabel: "Recent Work Note",
        value: (_task, _page, _index, item) => {
            const log = item?.latestWorkLog;
            if (!log) return "";
            if (log.dateTime && log.text) return `[${log.dateTime}] ${log.text}`;
            return log.text || log.dateTime || "";
    ...
    { key: "serviceNow", label: "ServiceNow", jiraLabel: "ServiceNow", value: (_task, page) => page?.["서비스나우"] ?? "" },
    { key: "jira", label: "Jira", jiraLabel: "Jira", value: (_task, page) => page?.jira ?? "" },
    { key: "workingNotes", label: "Work Notes (워킹노트)", jiraLabel: "Work Notes", value: (_task, page) => page?.["워킹노트"] ?? "" },
    { key: "bs", label: "BS", jiraLabel: "BS", value: (_task, page) => page?.BS ?? "" },
    { key: "fs", label: "FS", jiraLabel: "FS", value: (_task, page) => page?.FS ?? "" },
    { key: "ds", label: "DS", jiraLabel: "DS", value: (_task, page) => page?.DS ?? "" },
    { key: "ut", label: "UT", jiraLabel: "UT", value: (_task, page) => page?.ut ?? "" },
    { key: "relatedDocument", label: "Related Documents (관련 문서)", jiraLabel: "Related Documents", value: (_task, page) => page?.["관련 문서"] ?? "" },
    { key: "testDocument", label: "Test Documents (테스트 문서)", jiraLabel: "Test Documents", value: (_task, page) => page?.["테스트 문서"] ?? "" },
    { key: "created", label: "Created Date (생성일)", jiraLabel: "Created Date", value: (_task, page) => page?.created ?? "" },
    { key: "updated", label: "Updated Date (수정일)", jiraLabel: "Updated Date", value: (_task, page) => page?.updated ?? "" }
];

function jiraExportValue(field, task, index) {
    const item = pages.find(entry => entry.page?.file?.path === task.filePath) || null;
    const page = item?.page || null;
    const value = field.value(task, page, index, item);
...
function openJiraExportModal() {
    const JIRA_ENGLISH_LABEL_MAP = {
        "배포일": "Deploy Date",
        "Actual Release Date": "Actual Release Date",
        "Estimated QA Completion Date": "Estimated QA Completion Date",
        "Target QA Completion Date": "Target QA Completion Date",
        "작업예정일": "Scheduled Date",
        "마지막확인": "Last Checked",
        "업무 Priority": "Priority",
        "상태": "Status",
        "구분": "Category",
        "최근 작업일지": "Recent Work Note",
        "워킹노트": "Work Notes",
        "관련 문서": "Related Documents",
        "테스트 문서": "Test Documents",
        "생성일": "Created Date",
        "수정일": "Updated Date",
        "ServiceNow 생성일": "ServiceNow Created",
        "ServiceNow 수정일": "ServiceNow Updated",
        "To-Do": "To-Do"
    };

    columns.forEach(col => {
        if (!JIRA_EXPORT_FIELDS.some(f => f.key === col.key)) {
            const englishLabel = JIRA_ENGLISH_LABEL_MAP[col.label] || col.label;
            JIRA_EXPORT_FIELDS.push({
                key: col.key,
                label: englishLabel !== col.label ? `${englishLabel} (${col.label})` : col.label,
                jiraLabel: englishLabel,
                value: (_task, page, _index, item) => col.value(page, item)
            });
        }
    });
    ...
    dateRange.className = "opus-jira-export-date-range";
    ...
    dateRange.append(startLabel, endLabel);
    ...
    const fieldPanel = document.createElement("section");
    fieldPanel.className = "opus-jira-export-panel";
    fieldPanel.innerHTML = '<h4>2. 필드 선택 및 순서</h4>';
    const translationBox = document.createElement("div");
    translationBox.className = "opus-jira-export-translation";
    ...
    body.append(taskPanel, fieldPanel, previewPanel);
    modal.appendChild(body);
    const actions = document.createElement("div");
    actions.className = "opus-todo-create-actions";
    ...
    function syncTranslationOption() {
        ...
        if (!visible) {
            translateTodoToEnglish = false;
            translationCheckbox.checked = false;
        }
    }
    async function ensureEnglishTranslations() {
        if (!translateTodoToEnglish || !needsTodoTranslation()) return true;
        const plugin = app.plugins.getPlugin("servicenow-manage");
        if (typeof plugin?.translateTextForExport !== "function") {
            new Notice("ServiceNow Manage 또는 Translate 플러그인을 다시 활성화한 뒤 시도해 주세요.");
            return false;
        }
        translationWorking = true;
        translationBox.classList.add("is-working");
        translationCheckbox.disabled = true;
        copyButton.disabled = true;
        translationHelp.textContent = "영어 번역 중입니다. 선택한 To-Do 수에 따라 시간이 걸릴 수 있습니다.";
        try {
            ...
            translationHelp.textContent = "영어 번역이 적용되었습니다. 원본 To-Do는 변경되지 않습니다.";
            return true;
        } catch (error) {
            translateTodoToEnglish = false;
            translationCheckbox.checked = false;
            translationHelp.textContent = "번역을 적용하지 못했습니다. Translate 설정을 확인해 주세요.";
            new Notice(`Jira Export 영어 번역 실패: ${error.message || error}`, 9000);
            return false;
        } finally {
            translationWorking = false;
            translationBox.classList.remove("is-working");
    ...
    function renderTasks() {
        ...
        for (const [ticketId, tasks] of groups) {
            ...
            tasks.forEach(task => {
                const row = document.createElement("div");
                row.className = "opus-jira-export-task-row";
                const label = document.createElement("label");
                ...
                text.className = "opus-jira-export-task-text";
                ...
                if (text.textContent.length > 90) {
                    ...
                    more.type = "button";
                    more.className = "opus-jira-export-task-more";
...
function renderTodoBoard() {
    ...
    dateFilter.className = "opus-todo-date-filter";
    const dateBasis = document.createElement("select");
    [
        ["created", "등록일"],
        ["due", "완료 예정일"],
        ["completed", "완료일"]
        ...
        option.value = value;
        option.textContent = label;
        option.selected = settings.todoDateBasis === value;
        dateBasis.appendChild(option);
    });
    const dateFrom = document.createElement("input");
    dateFrom.type = "date";
    dateFrom.title = "시작일";
    dateFrom.value = settings.todoDateFrom || "";
    const dateSeparator = document.createElement("span");
    dateSeparator.className = "opus-todo-date-separator";
    dateSeparator.textContent = "~";
    const dateTo = document.createElement("input");
    dateTo.type = "date";
    dateTo.title = "종료일";
    dateTo.value = settings.todoDateTo || "";
    const clearDate = document.createElement("button");
    clearDate.className = "opus-todo-date-clear";
    clearDate.textContent = "날짜 지우기";
    const updateDateFilter = () => {
        settings.todoDateBasis = dateBasis.value;
        settings.todoDateFrom = dateFrom.value;
        settings.todoDateTo = dateTo.value;
        saveSettings();
        resetTodoVisibleLimits();
        renderTodoBoard();
    };
    dateBasis.addEventListener("change", updateDateFilter);
    dateFrom.addEventListener("change", updateDateFilter);
    dateTo.addEventListener("change", updateDateFilter);
    clearDate.addEventListener("click", () => {
        settings.todoDateFrom = "";
        settings.todoDateTo = "";
        saveSettings();
        resetTodoVisibleLimits();
        renderTodoBoard();
    });
    dateFilter.append(dateBasis, dateFrom, dateSeparator, dateTo, clearDate);
    boardFilters.append(search, quickView, dateFilter, guide);
    ...
    for (const status of TODO_STATUSES) {
        ...
        const isCollapsed = settings.todoCollapsedColumns.includes(status.key);
        column.classList.toggle("is-collapsed", isCollapsed);

        const header = document.createElement("div");
        header.className = "opus-todo-column-header";
        const toggle = document.createElement("button");
        toggle.className = "opus-todo-column-toggle";
        toggle.type = "button";
        toggle.title = isCollapsed ? `${status.label} 펼치기` : `${status.label} 접기`;
        const chevron = document.createElement("span");
        chevron.className = "opus-todo-column-chevron";
        ...
        toggle.addEventListener("click", () => {
            const collapsed = new Set(settings.todoCollapsedColumns);
            collapsed.has(status.key) ? collapsed.delete(status.key) : collapsed.add(status.key);
            settings.todoCollapsedColumns = [...collapsed];
            saveSettings();
            renderTodoBoard();
        });
